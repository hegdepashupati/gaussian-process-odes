...
def plot_model_initialization(model, data, args, name, make_plot=False):
    ...
    ts = torch.cat([ts, 2 * ts[-1:] - ts[-2:-1]])
    with torch.no_grad():
        for _ in range(args.eval_sample_size):
            with torch.no_grad():
                pred_ys.append(model(model.state_distribution.x0.sample()[0], ts))
                pred_xs.append(model.state_distribution.sample()[0])
...
def plot_latents(model, data, args):
    train_latent_mean = torch2numpy(model.state_distribution.mean())
    train_latent_poststd = torch.diagonal(model.state_distribution.lchol(), dim1=2, dim2=3)
    train_latent_postvar = torch2numpy(train_latent_poststd ** 2)
    train_constraint_var = torch2numpy(model.constraint.variance)
    train_latent_postvar_constvar = torch2numpy(train_latent_poststd ** 2) + train_constraint_var

    fig, axs = plt.subplots(1, 2, figsize=(8 * 2, 3 * 1))
    for d in range(2):
        axs[d].plot(data.trn.ts[1:], train_latent_mean[0, :, d], c='r', alpha=0.7, zorder=3)
        axs[d].fill_between(data.trn.ts[1:],
                            train_latent_mean[0, :, d] - 2.5 * train_latent_postvar[0, :, d] ** 0.5,
                            train_latent_mean[0, :, d] + 2.5 * train_latent_postvar[0, :, d] ** 0.5,
                            color='r', alpha=0.1, zorder=1)
        axs[d].fill_between(data.trn.ts[1:],
                            train_latent_mean[0, :, d] - 2.5 * train_latent_postvar_constvar[0, :, d] ** 0.5,
                            train_latent_mean[0, :, d] + 2.5 * train_latent_postvar_constvar[0, :, d] ** 0.5,
                            color='b', alpha=0.1, zorder=0)
        axs[d].scatter(data.trn.ts[1:], data.trn.ys[0, :-1, d], c='k', s=100, marker='.', zorder=200)
        axs[d].set_title("State {}".format(d + 1))
        axs[d].set_xlabel("Time")

        axs[d].scatter([], [], c='k', s=10, marker='.', label='train obs')
        axs[d].plot([], [], c='r', alpha=0.7, label='posterior mean')

    axs[-1].legend(loc='upper right')
    fig.suptitle("Latent state posteriors")
    fig.subplots_adjust()
    plt.show()
    fig.subplots_adjust(wspace=0.2, hspace=0.2)
    fig.savefig(os.path.join(args.save, 'plt_latents.png'), dpi=160,
                bbox_inches='tight', pad_inches=0.2)
    plt.close(fig)


def plot_longitudinal(data, test_pred, noisevar, args):
    ...
    for n in range(test_pred_mean.shape[0]):
        ...
        axs[-1].legend(loc="upper right")
        fig.suptitle("Predictive posterior for shooting GPODE")
        plt.show()
        fig.subplots_adjust(wspace=0.2, hspace=0.2)
...
def plot_vectorfield(model, data, test_pred, args):
    ...
    cs2 = ax2.contourf(xx, yy,
                       grid_f.std(0).mean(1).reshape(xx.shape),
                       levels=10,
                       cmap=cmap, alpha=0.6)
...
def plot_long_pred(data, pred, ts, args, name):
    nobs = min(pred.shape[1], 4)
    fig, axs = plt.subplots(nobs, 2, figsize=(8 * 2, 3 * nobs), sharex='all')
    for i in range(nobs):
        for j in range(2):
            pred_mean, pred_postvar = pred[:, i].mean(0), pred[:, i].var(0)
            axs[i, j].plot(torch2numpy(ts), pred_mean[:, j].T, c='r', alpha=0.5)
            axs[i, j].fill_between(torch2numpy(ts),
                                   pred_mean[:, j] - 2 * pred_postvar[:, j] ** 0.5,
                                   pred_mean[:, j] + 2 * pred_postvar[:, j] ** 0.5,
                                   color='r', alpha=0.1, zorder=1, label="posterior")
            axs[i, j].scatter(torch2numpy(ts), data[i, :, j], c='k', s=5, marker='.',
                              zorder=200)
            if i == 0:
                axs[i, j].set_title("state {}".format(j + 1))
            if i == (nobs - 1):
                axs[i, j].set_xlabel("Time")
        axs[i, 1].scatter([], [], c='k', s=10, marker='.', zorder=200, label='actual')
        axs[i, 1].plot([], [], c='r', alpha=0.2, label='predictive mean')
...
def plot_longnoise_pred(data, pred, noise_var, ts, args, name):
    ...
    for i in range(nobs):
        for j in range(2):
            axs[i, j].scatter(torch2numpy(ts), data[i, :, j], c='k', s=10, marker='.',
                              zorder=200)
            pred_mean, pred_postvar = pred[:, i].mean(0), pred[:, i].var(0)
            ...
            axs[i, j].fill_between(torch2numpy(ts),
                                   ...
                                   color='r', alpha=0.1, zorder=1, label="posterior")
            axs[i, j].fill_between(torch2numpy(ts),
                                   pred_mean[:, j] - 2 * pred_predvar[:, j] ** 0.5,
                                   pred_mean[:, j] + 2 * pred_predvar[:, j] ** 0.5,
                                   color='b', alpha=0.1, zorder=0, label="predictive")
            if i == 0:
                axs[i, j].set_title("state {}".format(j + 1))
...
def plot_long_pred_single(data, pred, ts, args, name):
...
def plot_longnoise_pred_single(data, pred, noise_var, ts, args, name):
    fig, axs = plt.subplots(1, 2, figsize=(16, 3), sharex='all')
    for j in range(2):
        axs[j].scatter(torch2numpy(ts), data[0, :, j], c='k', s=10, marker='.',
                       zorder=200)
...
def plot_trace(loss_meter, observ_nll_meter, inducing_kl_meter, state_kl_meter, args):
    ...
    ax4.plot(state_kl_meter.iters,
             state_kl_meter.vals)
    ax4.set_title("Latent KL")
    fig.subplots_adjust()
